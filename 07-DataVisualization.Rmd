...
These are the same data sets in two different graphs. The first one violates most of the rules for a bad graph (or follows most of the rules, I guess). The second one is better. It isn't pixelated and the text isn't stretched. The colors are visible even to a colorblind person (using *scale\_color\_colorblind()* from the *ggthemes* package). The axis labels are a bit more informative, giving the units for the sepal widths and lengths.
